...
### Changed
- **P0 final production data-integrity audit** — classified REAL, PAPER, DEMO, MOCK, and research sources repository-wide; removed the hardcoded `0.5` market-price fallback from Kelly sizing, made the remaining skeleton width deterministic, and confirmed production worker outages fail closed rather than switching to synthetic markets, balances, fills, traders, or PnL. Remaining simulation and mock paths are explicitly labeled and documented with live-trading blockers.
- **P1 REAL paper-execution calibration** — REAL recorder observations now produce a versioned calibration dataset with chronological train/out-of-sample splits, spread/depth/mid-move metrics, conservative maker-fill ranges, observed snapshot-latency ranges, and explicit fee/slippage assumptions. REAL paper backtests use the calibrated maker assumption and report `fill_model_version`, `calibration_dataset`, assumptions, maker/latency assumptions, fee model, slippage model, and OOS metrics. L2-only movement is explicitly treated as a proxy; exact queue position and unobserved fills are not claimed.
- **P0 explicit app-mode isolation** — `APP_MODE` now defaults to `PRODUCTION`; synthetic browser status is available only with explicit `APP_MODE=DEMO`. Missing worker configuration fails closed instead of generating demo markets, trader activity, balances, order counts, fills, or PnL. Dashboard mode badges persist as `DEMO MODE`, `PAPER MODE`, or `LIVE MODE`; analytics responses and cards expose their source and show `NO DATA` when unavailable.
- **P0 authoritative execution ledger** — worker ledger events now carry stable event IDs, lifecycle event types, token/quantity, execution mode, data source, version, and idempotency metadata. Durable JSONL writes complete before projection updates; duplicate events are ignored. Worker status exposes ledger health, last event, unresolved orders, stale state, reconciliation health, and execution authority through `/execution/health`, `/execution/stale`, and `/execution/reconcile`. PaperDesk execution/reset is blocked when worker authority is active, preventing an independent browser/Supabase execution truth. Live trading remains disabled.
- **P1 verified copy trading** — replaced the browser’s manual position-diff simulator with authenticated, normalized Polymarket Data API activity. Events retain wallet, market/condition, token, side, price/quantity, timestamp, source, and transaction identity; duplicates, stale events, missing markets, invalid quantities, and risk-limit violations are rejected. The UI shows `OBSERVED_TRADE`, `PAPER COPY`, and `NO DATA` states, never writes synthetic copy fills, and LIVE copy remains explicitly disabled. Added immutable Supabase activity-event storage with a uniqueness constraint.
- **P0 REAL ML training boundary** — production retraining now requires a provenance-bearing `REAL_HISTORICAL_POLYMARKET` dataset with `POLYMARKET_RESOLUTION` labels, rejects synthetic/legacy/Binance/simulated-fill inputs, performs chronological market-grouped train/validation/test splits, and persists dataset provenance, feature/label versions, and out-of-sample Brier/log-loss metrics in the model sidecar. Missing or invalid REAL data fails clearly without generating replacements.
- **P0/P1 browser Polymarket market maker** — replaced the Binance price proxy with the public Polymarket CLOB market WebSocket, subscribed by `token_id` and paired with a user-supplied `market_id`. The desk now renders real best bid/ask, spread, full-side depth, midpoint, update timestamp, and stale state; it fails closed as `OFFLINE / NO DATA` without inventing prices. All displayed orders are labeled `PAPER` and all fills `SIMULATED`; no browser MM action is represented as an actual Polymarket fill. The legacy Binance hook is explicitly named `Research Simulation — Binance/Generic` and is disconnected from production MM.
...
- **Paper Desk schema alignment** — corrected migration ordering for `paper_orders` and `paper_order_fills` while retaining the singular `paper_account` schema.
- **Control Room demo fallback** — retained the demo fallback and surfaced Supabase paper-state errors instead of leaving the UI in an indefinite connecting state.
- **Public Risk API** — documented the deployed sidecar URL at `https://polymarket-quant-bot-lite-1.onrender.com`.
- **Observed-book paper fills** — normalized feed-shaped and execution-shaped books, and required real observed CLOB depth for Risk API paper orders and trailing-stop closes.
