...
def place_order(*, market_slug, token_id, side, price, confidence, balance, user_id=1, db, category=None, reason="api", executor=None):
    ...
    try:
        outcome_side = Side(side.upper()) if side.upper() in ("UP", "DOWN") else Side.UP
    except AttributeError:
        outcome_side = Side.UP
    ...
    fill = fills[0]
    trade = Trade(market_slug=market_slug, category=category, side=str(side), entry_price=fill.avg_price, size_usd=fill.cost, status="open", dry_run=fill.simulated, order_id=fill.order_id, reason=reason)
    db.add(trade)
    db.commit()
    db.refresh(trade)
